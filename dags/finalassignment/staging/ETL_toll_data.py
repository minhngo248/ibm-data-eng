...
extract_data_from_csv = BashOperator(
    ...
    bash_command=f'''
        sudo echo Rowid,Timestamp,Anonymized_Vehicle_number,Vehicle_type > {saved_path} & 
        sudo cut -d ',' -f 1,2,3,4 {path_vehicle} >> {saved_path}
    ''',
    dag=toll_dag,
)
...
extract_data_from_tsv = BashOperator(
    ...
    bash_command=f'''
        sudo echo toll_plaza_id,nb_axles,toll_code > {saved_path_tollplaza} & 
        sudo awk \'{{printf "%s,%s,%s\n", $1, $(NF-2), $NF}}\' {path_tollplaza} >> {saved_path_tollplaza}
    ''',
    dag=toll_dag,
)
...
extract_data_from_fixed_width = BashOperator(
    ...
    bash_command=f'''sudo echo type_payment_code,vehicule_code > {saved_path_payment} & 
                sudo awk \'{{printf "%s,%s\n", $(NF-1), $NF}}\' {path_payment} >> {saved_path_payment}''',
    dag=toll_dag,
)

tmp_path = '/home/project/airflow/dags/finalassignment/tmp_data.csv'
saved_path_consolidated = '/home/project/airflow/dags/finalassignment/extracted_data.csv'

consolidate_data = BashOperator(
    task_id='consolidate_data',
    bash_command=f'''sudo paste -d ',' {saved_path} {saved_path_tollplaza} > {tmp_path} &
                sudo paste -d ',' {tmp_path} {saved_path_payment} > {saved_path_consolidated}''',
    dag=toll_dag,
)
...
transform_data = BashOperator(
    ...
    bash_command=f'''sudo awk \'BEGIN {{FS = OFS = ","}} {{printf "%s,%s,%s,%s,%s,%s,%s,%s\n", 
